...
print("Opening viewer -- the arm will cycle through a few preset points.")
print(f"Target 1/{len(WAYPOINTS)}: {target}")
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    while viewer.is_running():
        ...
        if dist < 0.01:
            ...
            target = WAYPOINTS[waypoint_idx]
            model.body_pos[marker_body_id] = target
            print(f">>> Reached! Moving to target {waypoint_idx + 1}/{len(WAYPOINTS)}: {target}")
